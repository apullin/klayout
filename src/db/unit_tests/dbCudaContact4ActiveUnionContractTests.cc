...
struct ContractFixture
{
      ...
      }
  {
    ...
    request.opcode = KLAYOUT_CUDA_SPATIAL_CONTACT4_ACTIVE_UNION_EMPTY;
    ...
    set_scene (
      ...
      KLAYOUT_CUDA_SPATIAL_CONTACT4_ACTIVE_DIGEST_DOMAIN, 0x3a);
    set_scene (
      request.contact, KLAYOUT_CUDA_SPATIAL_CONTACT4_CONTACT_ROLE, 10,
      KLAYOUT_CUDA_SPATIAL_CONTACT4_CONTACT_DIGEST_DOMAIN, 0x7c);
    ...
    request.max_contact_edges = 8;
    request.max_grid_cells = 16;
    request.max_contact_memberships = 64;
    request.max_boundary_cell_visits = 64;
    request.max_member_visits = 64;
    request.max_pair_work = 64;
    request.max_cells_per_contact_edge = 8;
    request.max_cells_per_boundary_edge = 8;
    ...
    result.contact_expanded_edge_count = 4;
    result.grid_cell_count = 1;
    result.contact_membership_count = 4;
    //  ACTIVE boundary segments outside the clipped CONTACT grid contribute
    //  zero visits.  This remains a complete exact proof, not an uncertainty.
    result.boundary_cell_visit_count = 0;
    ...
    result.contact_h2d_ns = 5;
    result.contact_expand_ns = 10;
    result.boundary_preflight_ns = 10;
  ...
  void set_scene (
    klayout_cuda_spatial_contact4_active_union_scene_v1 &scene,
    uint32_t role, uint32_t layer, const char *domain, uint8_t digest_byte)
  {
  ...
  static void set_echo (
    ...
    klayout_cuda_spatial_contact4_active_union_scene_echo_v1 &echo)
  {
...
static_assert (
  ...
  "CONTACT.4 scene ABI size changed");
static_assert (
  sizeof (klayout_cuda_spatial_contact4_active_union_scene_echo_v1) == 192,
  "CONTACT.4 scene echo ABI size changed");
static_assert (
  sizeof (klayout_cuda_spatial_contact4_active_union_request_v1) == 760,
  "CONTACT.4 request ABI size changed");
static_assert (
  sizeof (klayout_cuda_spatial_contact4_active_union_result_v1) == 944,
  "CONTACT.4 result ABI size changed");
static_assert (
  offsetof (
    klayout_cuda_spatial_contact4_active_union_request_v1, active) == 48,
  "CONTACT.4 ACTIVE descriptor offset changed");
static_assert (
  offsetof (
    klayout_cuda_spatial_contact4_active_union_request_v1, contact) == 328,
  "CONTACT.4 CONTACT descriptor offset changed");
...
TEST(1_QualifiedCompleteProof)
{
  ...
  EXPECT_EQ (
    std::memcmp (
      fixture.request.active.digest_domain, "KARAW001",
      KLAYOUT_CUDA_SPATIAL_CONTACT4_DIGEST_DOMAIN_BYTES),
    0);
  EXPECT_EQ (
    std::memcmp (
      fixture.request.contact.digest_domain, "KCRAW001",
      KLAYOUT_CUDA_SPATIAL_CONTACT4_DIGEST_DOMAIN_BYTES),
    0);
}

TEST(2_EchoStatusAndCapacityFailures)
{
  std::string error;

  ContractFixture bad_digest;
  ++bad_digest.result.active.scene_digest [0];
  EXPECT_EQ (
    db::cuda_spatial_validate_contact4_active_union_result (
      bad_digest.request, bad_digest.result, KLAYOUT_CUDA_SPATIAL_OK, &error),
    false);
  EXPECT_EQ (
    error,
  ...
  bad_capacity.result.boundary_cell_visit_count =
    bad_capacity.request.max_boundary_cell_visits + 1;
  EXPECT_EQ (
    db::cuda_spatial_validate_contact4_active_union_result (
  ...
  EXPECT_EQ (
    error,
    "CUDA CONTACT.4 ACTIVE-union backend returned impossible proof counters");
}

TEST(3_DispositionAndTelemetryFailures)
{
  std::string error;

  ContractFixture inconsistent_complete;
  inconsistent_complete.result.raw_hit_count = 1;
  inconsistent_complete.result.candidate_pair_count = 1;
  EXPECT_EQ (
    db::cuda_spatial_validate_contact4_active_union_result (
      inconsistent_complete.request, inconsistent_complete.result,
      KLAYOUT_CUDA_SPATIAL_OK, &error),
    false);
  EXPECT_EQ (
    error,
  ...
  ContractFixture valid_raw_hit;
  valid_raw_hit.result.disposition =
    KLAYOUT_CUDA_SPATIAL_CONTACT4_ACTIVE_UNION_RAW_HITS;
  valid_raw_hit.result.raw_hit_count = 1;
  valid_raw_hit.result.candidate_pair_count = 1;
  EXPECT_EQ (
    db::cuda_spatial_validate_contact4_active_union_result (
      valid_raw_hit.request, valid_raw_hit.result,
      KLAYOUT_CUDA_SPATIAL_OK, &error),
    true);

  ContractFixture bad_memory;
  ++bad_memory.result.callback_incremental_peak_bytes;
  EXPECT_EQ (
    db::cuda_spatial_validate_contact4_active_union_result (
      bad_memory.request, bad_memory.result,
      KLAYOUT_CUDA_SPATIAL_OK, &error),
    false);
  EXPECT_EQ (
    error,
  ...
  ContractFixture bad_timing;
  bad_timing.result.query_ns = bad_timing.result.total_ns + 1;
  EXPECT_EQ (
    db::cuda_spatial_validate_contact4_active_union_result (
      bad_timing.request, bad_timing.result,
      KLAYOUT_CUDA_SPATIAL_OK, &error),
    false);
  EXPECT_EQ (
    error,
...
TEST(4_FixedDigestDomainsFailClosed)
{
  ContractFixture fixture;
  std::string error;
  std::memcpy (
    fixture.request.active.digest_domain,
    KLAYOUT_CUDA_SPATIAL_CONTACT4_CONTACT_DIGEST_DOMAIN,
    KLAYOUT_CUDA_SPATIAL_CONTACT4_DIGEST_DOMAIN_BYTES);
  ContractFixture::set_echo (fixture.request.active, fixture.result.active);
  EXPECT_EQ (
    db::cuda_spatial_validate_contact4_active_union_result (
      fixture.request, fixture.result, KLAYOUT_CUDA_SPATIAL_OK, &error),
    false);
