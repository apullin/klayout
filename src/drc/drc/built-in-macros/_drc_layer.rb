...
CODE
    ...
    # Internal, fail-closed acceleration hook that forms the exact physical
    # NWELL/PWELL union on the selected device and checks its resident
    # boundary against pristine ACTIVE.  False requires the exact historical
    # WELL union and ACTIVE.3 expression.
    def cuda_active3_well_union_clean?(pwell, active)
      @engine._context("cuda_active3_well_union_clean?") do
        check_is_layer(pwell)
        check_is_layer(active)
        requires_region
        pwell.requires_region
        active.requires_region
        self.data.cuda_active3_well_union_clean?(pwell.data, active.data)
      end
    end
