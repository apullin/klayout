...
bool classify_batch(const EdgePair *pairs, std::size_t count,
  ...
  if (!cuda_ok(cudaMalloc(&device_pairs, pair_bytes),
               "cudaMalloc(pairs)", error)) {
    return false;
  }
  ...
  bool ok = cuda_ok(
      cudaMemcpy(device_pairs, pairs, pair_bytes, cudaMemcpyHostToDevice),
      "cudaMemcpy(pairs H2D)", error);
  if (ok) {
    constexpr unsigned int kBlockSize = 256;
    constexpr std::size_t kMaximumGridX = 65535;
    const std::size_t needed = (count - 1) / kBlockSize + 1;
    const unsigned int blocks = static_cast<unsigned int>(
        needed < kMaximumGridX ? needed : kMaximumGridX);
    classify_kernel<<<blocks, kBlockSize>>>(
        device_pairs, count, distance, device_results);
    ok = cuda_ok(cudaGetLastError(), "classify_kernel launch", error);
  }
  if (ok) {
    ok = cuda_ok(
        cudaMemcpy(results, device_results, result_bytes,
  ...
  if (ok && !cuda_ok(free_results, "cudaFree(results)", error)) ok = false;
  if (ok && !cuda_ok(free_pairs, "cudaFree(pairs)", error)) ok = false;
  return ok;
}

const char *verdict_name(Verdict verdict) {
  switch (verdict) {
    case Verdict::kNoViolation: return "NO_VIOLATION";
