...
// every cut must be contained by one raw metal rectangle with the required
// margin on both X sides or on both Y sides.  A raw rectangle is a subset of
// the merged metal region, so success is conservative.  Non-rectangular
// metal templates are ignored; a non-rectangular cut declines the scene.
...
constexpr std::uint64_t kDefaultMaxBruteforceBoxPairs =
    UINT64_C(10000000);
...
bool is_rectangle_template(const LoadedScene &scene,
  ...
  for (std::uint32_t local = 0; local < polygon.edge_count; ++local) {
    ...
    if ((!right && edge.x1 != polygon.bbox[0]) ||
        (!top && edge.y1 != polygon.bbox[1])) {
      return false;
    }
...
ProjectionBox polygon_box(const PolygonRecord &polygon) {
  return {polygon.bbox[0], polygon.bbox[1],
          polygon.bbox[2], polygon.bbox[3]};
}

std::vector<ProjectionBox> decompose_manhattan_polygon(
    const LoadedScene &scene, const PolygonRecord &polygon) {
  ...
  for (std::size_t slab = 0; slab + 1 < ys.size(); ++slab) {
    ...
    for (std::size_t crossing = 0; crossing < crossings.size();
         crossing += 2) {
      ...
      rectangle_area +=
          static_cast<__int128>(right - left) * (top - bottom);
    }
  }
...
ProjectionLowered lower_projection_scene(const LoadedScene &scene,
  ...
  for (std::uint64_t cell_id = 0; cell_id < scene.header.cell_count;
    ...
    for (std::uint64_t local = 0; local < source_cell.polygon_count; ++local) {
      ...
      if (is_rectangle_template(scene, polygon)) {
        ...
        const std::vector<ProjectionBox> decomposition =
            decompose_manhattan_polygon(scene, polygon);
        if (decomposition.size() >
            std::numeric_limits<std::uint32_t>::max() -
                cell.metal_count) {
          throw SceneError(
              "per-cell decomposed metal count exceeds uint32");
        }
        lowered.templates.insert(lowered.templates.end(),
                                 decomposition.begin(),
                                 decomposition.end());
        cell.metal_count +=
            static_cast<std::uint32_t>(decomposition.size());
  ...
  for (std::uint32_t context_id = 0;
    ...
    const ProjectionCell &cell =
        lowered.cells[lowered.contexts[context_id].cell];
    if (cell.metal_count) {
      lowered.metal_contexts.push_back(context_id);
      lowered.metal_offsets.push_back(lowered.metal_count);
      if (!checked_add_u64(lowered.metal_count, cell.metal_count,
                           &lowered.metal_count) ||
          lowered.metal_count > max_metal_boxes) {
        throw SceneError(
            "logical metal rectangle count exceeds configured capacity");
      }
    }
    if (cell.cut_count) {
      lowered.cut_contexts.push_back(context_id);
      if (!checked_add_u64(lowered.cut_count, cell.cut_count,
                           &lowered.cut_count) ||
          lowered.cut_count > max_cut_boxes) {
        throw SceneError(
            "logical cut rectangle count exceeds configured capacity");
      }
    }
  }
  if (!lowered.metal_count || !lowered.cut_count) {
    throw SceneError(
        "projection-enclosure scene requires metal and cut rectangles");
  }
...
__device__ bool transform_projection_box_checked(
    const ContextGpu &context, const ProjectionBox &source,
    ProjectionBox *destination) {
...
ProjectionBox transform_projection_box_host(const ContextGpu &context,
                                             const ProjectionBox &source) {
  const std::int64_t xs[4] = {
      source.left, source.left, source.right, source.right};
  const std::int64_t ys[4] = {
      source.bottom, source.top, source.bottom, source.top};
  ProjectionBox transformed = {
      ...
      std::numeric_limits<std::int64_t>::min()};
  for (int corner = 0; corner < 4; ++corner) {
    const auto point = transform_128(context.transform, xs[corner],
                                     ys[corner]);
    const std::int64_t x =
        narrow_i64(point.first + context.tx, "projection box x");
    const std::int64_t y =
        narrow_i64(point.second + context.ty, "projection box y");
...
__device__ bool projection_box_span(const ProjectionBox &box,
                                    const Grid &grid,
                                    std::int64_t *x0,
                                    std::int64_t *y0,
                                    std::int64_t *x1,
...
__device__ bool projection_certificate(const ProjectionBox &metal,
                                       const ProjectionBox &cut,
                                       std::int64_t distance) {
  const bool contained_x =
      metal.left <= cut.left && metal.right >= cut.right;
  const bool contained_y =
      metal.bottom <= cut.bottom && metal.top >= cut.top;
  if (!contained_x || !contained_y) {
    return false;
  }
  const bool x_margin =
      cut.left - metal.left >= distance &&
      metal.right - cut.right >= distance;
  const bool y_margin =
      cut.bottom - metal.bottom >= distance &&
      metal.top - cut.top >= distance;
  return x_margin || y_margin;
}

bool projection_certificate_host(const ProjectionBox &metal,
                                 const ProjectionBox &cut,
                                 std::int64_t distance) {
  const bool contained_x =
      metal.left <= cut.left && metal.right >= cut.right;
  ...
  return (cut.left - metal.left >= distance &&
          metal.right - cut.right >= distance) ||
         (cut.bottom - metal.bottom >= distance &&
          metal.top - cut.top >= distance);
}

__global__ void projection_transform_gate(std::uint32_t *status) {
  ...
  if (!transform_projection_box_checked(context, source, &transformed) ||
      transformed.right - transformed.left !=
          ((code & 1u) ? 60 : 40) ||
      transformed.top - transformed.bottom !=
          ((code & 1u) ? 40 : 60)) {
    atomicOr(status, static_cast<std::uint32_t>(kInvalidDeviceRecord));
  }
}

__global__ void expand_projection_metal_kernel(
    const ContextGpu *contexts, const std::uint32_t *metal_contexts,
    const std::uint64_t *metal_offsets, const ProjectionCell *cells,
    const ProjectionBox *templates, std::uint32_t context_count,
    ProjectionBox *metal_boxes, std::uint32_t *status) {
  ...
  for (std::uint32_t local = threadIdx.x; local < cell.metal_count;
       local += blockDim.x) {
    ProjectionBox box{};
    if (!transform_projection_box_checked(
            context, templates[cell.metal_begin + local], &box)) {
...
__global__ void count_projection_grid_kernel(
    const ProjectionBox *metal_boxes, std::uint32_t metal_count,
    Grid grid, std::uint32_t *counts, unsigned long long *total,
    std::uint32_t *status) {
  for (std::uint32_t id = blockIdx.x * blockDim.x + threadIdx.x;
       id < metal_count; id += blockDim.x * gridDim.x) {
    std::int64_t x0 = 0;
    std::int64_t y0 = 0;
    std::int64_t x1 = 0;
    std::int64_t y1 = 0;
...
__global__ void fill_projection_grid_kernel(
    const ProjectionBox *metal_boxes, std::uint32_t metal_count,
    Grid grid, std::uint32_t *cursors, std::uint32_t *members,
...
__global__ void query_projection_cuts_kernel(
    ...
    std::int64_t distance, ProjectionCounters *counters,
    ProjectionSample *samples, std::uint32_t *sample_count,
    std::uint32_t *status) {
  const std::uint32_t list_id = blockIdx.x;
  ...
  for (std::uint32_t local = threadIdx.x; local < cell.cut_count;
    ...
    if (certified) {
      ...
      if (sample < kSampleCapacity) {
        samples[sample] = {
            context_id, local, cut.left, cut.bottom, cut.right, cut.top};
      }
    }
  }
  if (local_cuts) {
...
struct ProjectionOptions {
  ...
  std::uint64_t max_bruteforce_pairs =
      kDefaultMaxBruteforceBoxPairs;
};

ProjectionOptions parse_projection_options(int argc, char **argv) {
...
ProjectionBruteforce verify_projection_bruteforce(
    const ProjectionLowered &lowered, std::uint64_t maximum_pairs) {
  std::uint64_t pair_count = 0;
  if (!checked_mul_u64(lowered.metal_count, lowered.cut_count,
                       &pair_count) ||
      pair_count > maximum_pairs) {
    throw SceneError(
        "projection CPU brute-force pair count exceeds capacity");
  }
  ...
  for (const ProjectionBox &cut : cuts) {
    ...
    for (const ProjectionBox &metal : metals) {
      ...
      if (projection_certificate_host(
              metal, cut, kProjectionDistance)) {
        certified = true;
        break;
      }
    }
...
int run_projection_plan(const ProjectionOptions &options,
  ...
  if (options.expected_scene_sha256 !=
  ...
  if (options.verify_bruteforce) {
    begin = Clock::now();
    brute_force = verify_projection_bruteforce(
        lowered, options.max_bruteforce_pairs);
    end = Clock::now();
  ...
  const std::int64_t base_x = host_floor_div(root_metal.left, kGridCell);
  const std::int64_t base_y = host_floor_div(root_metal.bottom, kGridCell);
  const std::int64_t maximum_x =
      host_floor_div(root_metal.right, kGridCell);
  const std::int64_t maximum_y =
      host_floor_div(root_metal.top, kGridCell);
  ...
  DeviceBuffer<std::uint64_t> d_metal_offsets(
      lowered.metal_offsets.size());
  ...
  begin = Clock::now();
  expand_projection_metal_kernel<<<static_cast<unsigned int>(
                                       lowered.metal_contexts.size()),
                                   128>>>(
  ...
  if (host_status) {
    throw SceneError("device metal expansion declined the scene, flags=" +
  ...
  if (!membership_total || membership_total > options.max_memberships ||
    ...
    throw SceneError(
        "projection grid memberships exceed configured/uint32 capacity");
  }
  ...
  cuda_require(cudaMemcpy(d_offsets.get() + grid_cells, &terminal,
  ...
  query_projection_cuts_kernel<<<static_cast<unsigned int>(
                                     lowered.cut_contexts.size()),
                                 128>>>(
      ...
      d_counts.get(), d_offsets.get(), d_members.get(),
      kProjectionDistance, d_counters.get(), d_samples.get(),
  ...
  if (brute_force &&
      (brute_force->cuts != counters.cuts ||
       brute_force->certified != counters.certified ||
       brute_force->misses != counters.misses)) {
    throw SceneError(
        "projection GPU result disagrees with CPU brute force");
  }
  ...
  if (host_status) {
    verdict = "UNCERTAIN";
    exit_code = 2;
  } else if (counters.misses) {
    verdict = "FALLBACK";
    exit_code = 3;
  }
  std::cout << std::fixed << std::setprecision(3)
            << "PROJECTION_ENCLOSURE_GPU_ISLAND"
            << " verdict=" << verdict
...
int main(int argc, char **argv) {
  ...
  try {
    return run_projection_plan(
        parse_projection_options(argc, argv), total_begin);
