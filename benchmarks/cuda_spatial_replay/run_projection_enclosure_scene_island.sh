...
build_dir=${BUILD_DIR:-/tmp/klayout-projection-enclosure-island-build}
...
TMPDIR="${build_dir}/tmp" "${nvcc}" \
  ...
  "${script_dir}/projection_enclosure_scene_island.cu" \
  -o "${build_dir}/projection_enclosure_scene_island"

exec "${build_dir}/projection_enclosure_scene_island" "$@"
