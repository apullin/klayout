...
__global__ void classify_kernel(const CandidatePair *pairs,
                                std::size_t count,
...
bool classify_batch(const CandidatePair *pairs, std::size_t count,
  ...
  if (count > std::numeric_limits<std::size_t>::max() /
      ...
      count >
          std::numeric_limits<std::size_t>::max() / sizeof(Verdict)) {
    if (error) {
  ...
  CandidatePair *device_pairs = nullptr;
  Verdict *device_results = nullptr;
  const std::size_t pair_bytes = count * sizeof(CandidatePair);
