...
usage() {
  cat >&2 <<'EOF'
Usage:
  bash run_m2_via1_packed_scene_capture.sh \
...
The intermediate scene maps METAL2 to 101/0 and VIA1 to 102/0; the packed output
uses those same two slots in the existing KACTSCN1 format.
EOF
}

die() {
  echo "M2/VIA1 packed-scene capture: $*" >&2
  exit 2
}

here=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
capture_deck="${here}/m2_via1_projection_capture.lydrc"
...
work=$(mktemp -d "${TMPDIR:-/tmp}/klayout-m2-via1-capture.XXXXXX")
stage_dirs=()
published=()
...
new_stage_dir() {
  ...
  stage=$(mktemp -d "${parent}/.${base}.m2-via1-stage.XXXXXX")
  stage_dirs+=("${stage}")
  stage_result=${stage}
}
...
grep -Fq -- "M2_VIA1_CAPTURE" "${capture_log}" ||
  die "capture did not emit its completion marker"
grep -Fq -- "<name>METAL2.3</name>" "${temp_report}" ||
  die "source report is missing METAL2.3"

env "${common_env[@]}" "${klayout}" -b \
  -r "${exporter}" \
  -rd "input=${temp_scene}" \
  -rd "topcell=KLAYOUT_CUDA_M2_VIA1_SCENE" \
...
echo "M2/VIA1 packed-scene capture passed."
echo "report_items=${report_items}"
grep -F -- "M2_VIA1_CAPTURE" "${capture_log}" | tail -n 1
tail -n 1 -- "${export_log}"
