...
class RuntimeProvenanceTests(unittest.TestCase):
    ...
    def test_python_bytecode_cache_changes_runtime_identity(self) -> None:
        install = self.make_install("python-bytecode-cache")
        language = self.add_language_runtime(install)
        source = install["root"] / "pymod" / "adapter.py"
        ...
        bytecode = source.parent / "__pycache__" / "adapter.cpython-312.pyc"
        ...
        before_root = next(
            ...
            if "klayout-install-pymod" in root["roles"]
        )
        after_root = next(
            root
            for root in after["identity"]["language_runtime_roots"]
            if "klayout-install-pymod" in root["roles"]
        )
        self.assertEqual(
            after_root["file_count"], before_root["file_count"] + 1
        )
